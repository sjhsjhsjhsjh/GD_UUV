...
const ui = {
  ...
  toggleDepth50: document.getElementById('toggleDepth50'),
  toggleDepth100: document.getElementById('toggleDepth100'),
  toggleDepth150: document.getElementById('toggleDepth150'),
  toggleDepth200: document.getElementById('toggleDepth200'),
  toggleDepth250: document.getElementById('toggleDepth250'),
};
...
/**
 * 根据深度值（米）匹配到标准深度层级（50, 100, 150, 200, 250m）
 * 采用四舍五入匹配，容差为±25m
 * @param {number} depthM - 深度值（负数表示深度）
 * @returns {number|null} - 匹配的标准深度层级（50, 100, 150, 200, 250），或null表示不在标准层级
 */
function matchDepthLayer(depthM) {
  const absDepth = Math.abs(depthM);
  const standardDepths = [50, 100, 150, 200, 250];
  
  for (const depth of standardDepths) {
    if (Math.abs(absDepth - depth) <= 25) {
      return depth;
    }
  }
  return null;
...
function buildEnemySlice(enemyIndex) {
  ...
  for (let i = 0; i < item.tl_values.length; i += 1) {
    ...
    // 检查该点是否属于可见的深度层级
    const depthZ = Number(points.z_m[i]);
    const matchedDepth = matchDepthLayer(depthZ);
    if (matchedDepth === null || !state.depthVisibility[matchedDepth]) {
      continue;
    }
...
function toTlTrace(slice) {
  ...
  return {
    ...
    marker: {
      ...
      color: slice.color,
      colorscale: state.currentColorscale,
      cmin: slice.stats.min,
      cmax: slice.stats.max,
      colorbar: {
        title: 'TL (dB)',
        thickness: 14,
        len: 0.75,
...
async function loadAll() {
  ...
  // 从配置文件读取colorscale，若不存在则使用Plasma
  state.currentColorscale = String(runtimeConfig.plotly_config?.colorscale || 'Plasma');

  ui.tlCapInput.value = String(state.tlColorCapDb);
  ui.colorscaleSelect.value = state.currentColorscale;
...
function bindEvents() {
  ...
  // 深度层级过滤事件绑定
  [50, 100, 150, 200, 250].forEach((depth) => {
    const checkboxId = `toggleDepth${depth}`;
    const checkbox = ui[checkboxId];
    if (checkbox) {
      checkbox.addEventListener('change', () => {
        state.depthVisibility[depth] = checkbox.checked;
        drawEnemyByIndex(state.currentEnemyIndex);
      });
    }
  });
  ...
  // 配色方案选择事件绑定
  ui.colorscaleSelect.addEventListener('change', () => {
    state.currentColorscale = ui.colorscaleSelect.value;
    drawEnemyByIndex(state.currentEnemyIndex);
  });
